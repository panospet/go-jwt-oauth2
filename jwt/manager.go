...
func (m *JwtManager) Authenticate(
	ctx context.Context,
	tokenStr string,
) error {
	...
	_, err = m.Keeper.FetchAuth(ctx, access)
	if err != nil {
		return err
	}
	return nil
...
func (m *JwtManager) DeAuthenticate(
	ctx context.Context,
	tokenStr string,
) error {
	token, err := m.verifyToken(tokenStr)
	...
	return m.Keeper.DeleteByAccess(ctx, access)
}
...
func (m *JwtManager) CreateAndStoreTokens(
	ctx context.Context,
	userId string,
) (authkeep.JwtTokens, error) {
	tokens, err := m.createTokens(userId)
	if err != nil {
		return authkeep.JwtTokens{}, fmt.Errorf("cannot create token: %s", err)
	}

	if err := m.Keeper.AddAuth(ctx, userId, tokens); err != nil {
		return authkeep.JwtTokens{}, err
	}
...
func (m *JwtManager) RefreshTokens(
	ctx context.Context,
	refreshToken string,
) (authkeep.JwtTokens, error) {
	token, err := jwt.Parse(
	...
	if ok && token.Valid {
		...
		deleted, delErr := m.Keeper.DeleteAuthByUuid(ctx, refreshUuid)
		if delErr != nil || deleted == 0 {
			return authkeep.JwtTokens{}, fmt.Errorf("could not remove auth entry: %s", err)
		}

		return m.CreateAndStoreTokens(ctx, userId)
	}
